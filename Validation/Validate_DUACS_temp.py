...
#ncdir+=[setupdir[1] + 'no_nudge_combined/'] 		  #   directory of schism nc output 
										  # True: use station output False: extract from netcdf (slow)
 
setupdir+=['/gpfs/work/jacobb/data/SETUPS/NorthSea/NorthSea200mHelgoland/RERUN2/']*2  # schism run directory, containts hgrid.* etc.
ncdir+=[setupdir[2] + 'no_nudge_combined/'] 		  #   directory of schism nc output 
ncdir+=[setupdir[3] + 'combined_nudging_albedo/']

setupdir+=['/gpfs/work/jacobb/data/SETUPS/NorthSea/NorthSea200mHelgoland/RERUN3/']
ncdir+=[setupdir[4] + 'combined/']
setup_names=['dwd','dwd_ndge','era','era_alb','era_jrlv4']#['schism_dwd','schism_era']
...
outdir=setupdir[-1]+SateliteProdcut+'_all_'+varname+'/'		  # output directory where images will be stored
pic_format='.png'
dpivalue=300
