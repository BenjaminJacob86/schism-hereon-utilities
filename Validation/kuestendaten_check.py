...
MIN_DEPTH = 4 #m
#x[d<MIN_DEPTH]=np.inf  #not contiueed
...
if not np.any(valid):
    raise RuntimeError("No wet points available for tree building.")

coords_valid = np.column_stack((x[valid], y[valid]))
...
# Keep mapping from tree indices -> original indices
valid_idx = np.nonzero(valid)[0]  # valid_idx[i_tree] -> index in original arrays
# Example query: batch of query points qx,qy
query_points = np.column_stack((qx, qy))      # shape (n_queries, 2)
dists, idx = tree.query(station_coords, k=1)     # or k=N
# map back to original indices
nns=original_indices = valid_idx[idx]
