	...
	setup_names=['GBHRballje','gcoast']#['schism_dwd','schism_era']
	varname_model='temp'
	######################
	...
	use_gcoast=True						  # compare against amm15
	gcoastdir='/gpfs/work/jacobb/data/SETUPS/GermanBight/GermanBight_HR_Ballje/gcoast_copy/'
	varname_gcoast='votemper'
	###################
	...
	# corresponding cmems	
	if use_gcoast:	
		gcoastfiles=np.sort(glob.glob(gcoastdir+'*.nc'))
		#(np.concatenate([glob.glob(oceandir+'*'+pattern+'*{:d}*.nc'.format(year)) for year in years]))
		extend=[lonmin,lonmax,latmin,latmax]
		varnames+=[varname_gcoast]
		class cgcoast():
			def __init__(self,gcoastfiles,extend,t0,t1):
				self.nc = xr.open_mfdataset(gcoastfiles)
				d0=self.nc.depth[0]
				self.nc = self.nc.sel(lon=slice(extend[0],extend[1]),lat=slice(extend[2],extend[3]),time=slice(str(t0)[:19],str(t1)[:19]),depth=d0)
				self.LON,self.LAT=np.meshgrid(self.nc['lon'],self.nc['lat'])
				self.tree=cKDTree(list(zip(self.LON.flatten(),self.LAT.flatten())))
		gcoast=cgcoast(gcoastfiles,extend,t0,t1)			
		gcoast.nn=gcoast.tree.query(list(zip(LON.flatten(),LAT.flatten())))[1]
		gcoast.mask=s.mask
		setups[len(setups)]=gcoast
		setup_names+=['gcoast']
		
		## compute anomalies for schism
		if anomaly==True:
			print('computing temporal mean for anomalies for cmems')
			offsets+=[gcoast.nc[varname_model].mean(axis=0)]
		else:
			offsets+=[ np.zeros(gcoast.LON.shape)]
	...
	for file in SATfiles[:nt]:  # osisaf files  need to be iterated
		...
		for ti,timei in enumerate(time):
			...
			for i,s in enumerate(setups.values()):
				nci=s.nc.interp(time=str(timei)[:19])
				OBSi=(nci[varnames[i]].values.flatten()+offsets[i].flatten())[s.nn]
	...
	for j,variable in enumerate(variables):
		...
		for i,s in enumerate(setups.values()):
			if (j>0) & (j <3):
				vmin=np.minimum(vmin,np.nanquantile(variable[i][variable[i].mask==False],0.1))#np.nanmin(variable[i]))
				vmax=np.maximum(vmax,np.nanquantile(variable[i][variable[i].mask==False],0.99))#np.nanmax(variable[i]))
			else:
