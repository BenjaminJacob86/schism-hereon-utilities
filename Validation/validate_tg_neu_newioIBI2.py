...
sys.path.insert(0,'/work/gg0028/SCHISM/schism-hereon-utilities/Lib/')
...
ibi_dir='/work/gg0028/g260114/RUNS/GermanBight/GB_IBI_2024_inst_elev/elevIBI/'
...
# New API donwloaded IBI    
def get_file_start_dates2(files):
	start_dates=[]
	for i,file in enumerate(files):
		startdate=file[-13:-3]
		start_dates.append(pd.to_datetime(startdate, format='%Y-%m-%d')) #%H%M
	return(start_dates)    
...
if use_ibi:
	...
	wild_elev_files='*2D*' # * varying infprmation	
	...
	def reduce_file_list(files):
		...
		try:
			...
			file_dates=np.asarray(get_file_start_dates2(files))
	...
	try:
		...
		temp=xr.open_dataset(ssh_files[0])
		isnan=np.isnan(temp.zos[0,:].values)
		nt_file=len(temp.time)
		temp.close()
		
		dsIBI=xr.combine_nested([ xr.open_dataset(file)  for file in ssh_files],concat_dim='time',data_vars=['zos',])
		ntfiles=nt_file*len(ssh_files)
		dates_out=file_dates[0]+np.arange(ntfiles)*np.timedelta64(1,'h')+30*np.timedelta64(1,'m')
		lon2d,lat2d=np.meshgrid(dsIBI['longitude'].values,dsIBI['latitude'].values)
	...
	try:
		...
		stations['IBI']['zeta']=np.asarray([dsIBI['zos'][i,:].values[ii2,jj2] for i in range(len(dsIBI['time']))])    
	stations['IBI']['time']=dates_out
	#setup_names.append('IBI-nn')
	setup_names.append('IBI')
