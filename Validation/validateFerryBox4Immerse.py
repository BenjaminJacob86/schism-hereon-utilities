...
########## settings #################################
# directories
fbdir='/gpfs/work/ksddata/observation/insitu/FerryBox_Sebastian/' 					  # folder containing FerryBox nc tracks 
fbdir='/gpfs/work/jacobb/data/SETUPS/SNS_Wei/valid/FB/'

setupdir='/gpfs/work/chenw1/SCHISM/cfgs/Control_Run/SNS_GB_Combined/'  				  # schism rundir	
ncdir= '/gpfs/work/chenw1/SCHISM/cfgs/Control_Run/SNS_GB_Combined/outputs/Combined_drag_turned_1/'#setupdir+'combined/' 												  #	output dir
oceandir='/gpfs/work/jacobb/data/SETUPS/GermanBight/GB2018/amm15/' 									  # amm15 directory
outdir='/gpfs/work/jacobb/data/immerse/FerryBox/' # output directory where images will be stored

#NEMO Sebatian
nemodir='/gpfs/work/jacobb/data/immerse/nemo/'
masknc=Dataset(nemodir+'mask.nc')
LON=masknc['glamt'][0,:]
LAT=masknc['gphit'][0,:]
LandSeaMask=np.asarray(masknc['tmask'][0,0,:,:],bool)
sst=nc['sst'][0,:,:]
nc=Dataset(nemodir+'SNS_SS_2018.nc')
tnemo=nc['time_counter'][:]
utnemo=utime(nc['time_counter'].units)
nemodates=utnemo.num2date(tnemo)

LON2=LON.copy()
LON2[sst.mask | ~LandSeaMask ]=9999 # Trick nn search tree  (pdata==100)
...
for filenr,file in	enumerate(FBt): #FBt[10],: #
	...
	# nemo profiles
	Tintp3=np.zeros(len(T[iuse]))
	Sintp3=np.zeros(len(T[iuse]))
	tnni3=np.zeros(len(T[iuse]))
	...
	tfile=np.sort(glob.glob(oceandir+'*TEM*'+tname[tname.rindex('_'):] ))[0]
	sfile=np.sort(glob.glob(oceandir+'*SAL*'+tname[tname.rindex('_'):] ))[0]
	#moc=Amm15(oceandir+tname,oceandir+sname)
	#moc=Amm15(tfile,sfile)
	...
	for i,date in enumerate(fbdates[iuse]):
			...
			ti=np.argmin(np.abs(ammdates-date))
			nni=moc.tree.query((lon2[i],lat2[i]))
			Sintp3[i]=nc['sss'][ti,:].flatten()[nni]
			Tintp3[i]=nc['sst'][ti,:].flatten()[nni]
	
			# nemo
			ti=np.argmin(np.abs(nemodates-date))
			nni=np.argmin((LON2flat-lon2[i])**2 +(LATflat-lat2[i])**2)
			Sintp3[i]=nc['sss'][ti,:].flatten()[nni]
			Tintp3[i]=nc['sst'][ti,:].flatten()[nni]
	print('done interpolating nemo')			
...
#print('done interpolating AMM15')		
	...
	extract[filenr]['nemo']={'T':np.asarray(Tintp3),'S':np.asarray(Sintp3)}
	...
	# relative o middel of period
	tiref=np.int(np.round(len(fbdates)/2))
	date=fbdates[tiref]
