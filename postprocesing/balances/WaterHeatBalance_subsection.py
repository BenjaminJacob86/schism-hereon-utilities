...
run_name='BlackSea_merge_16_17'            # Name of Excelfile Carrying bilances
...
cwd=os.getcwd()
ncdir=setupdir+'outputs/'

outfolder='/sciclone/data10/bjacob01/BlackSea'+run_name+'/'
kmlfile='/sciclone/home20/bjacob01/git/schism-hzg-utilities/postprocesing/balances/OceanDB.kml'  #ocean regions kml file
...
########
years=range(2016,2016+1)     # years considered
reftime=dt.datetime(2016,6,1,0,0,0)#+dt.timedelta(days=214) # fake shift to end to have a fullyear
prec_years=np.arange(2016,2016+1)
evap_years=np.arange(2016,2016+1)
...
#BasinStraits['Mediterranean Sea']=[('Gibraltar',1)]   #+1 :flows into, -1: flows out (higher to lower number in fluxflag.prop)
...
def get_annual_mean_river_runoff(s,areas,years,startdate=dt.datetime(2007,6,1)):
	...
	if os.path.exists('flux.th'):
		...
		with open('bctides.in','r') as f:
			while len(line)>0:
				...
				isriver=False
				if '4 4 4 4' in line:
					flux_indices.append(np.nan)
					bd_is_river.append(False)
					continue
				if 'river' in line:
				...
				if ('0 2 1 2' in line):
					line=f.readline()
					fixed_values.append(np.float(line))
					flux_indices.append(np.nan)
					bd_is_river.append(True)
				elif isriver:
					fixed_values.append(np.nan)
					fluxind+=1
					flux_indices.append(fluxind)
					bd_is_river.append(True)
		...
		for ifile,basin in enumerate(areas):
				...
				if len(basinbdrivers[basin][0]) > 0:
					bc_indices[basin]=basinbdrivers[basin]-obdtype[:basinbdrivers[basin][0][0]].count('ocean')+1 # one for time
				else:
					bc_indices[basin]=np.nan
	else:
		bd_rnames=[]
		Ryears=[]
	...
	bd_is_river=np.asarray(bd_is_river)
	fixed_values=np.asarray(fixed_values)
	flux_inds=dict.fromkeys(areas)
	const_flow=dict.fromkeys(areas)
	for basin in areas:
			if os.path.exists('flux.th'):
				#from_bdriver=-np.mean(q[Ryears==years[0],:][:,np.asarray(basinbdrivers[basin])-4+1],axis=0)[0]
				from_bdriver=np.zeros(len(basinbdrivers[basin][0]))
				fval=fixed_values[basinbdrivers[basin][0]-1] # constant river forcing
				from_bdriver[~np.isnan(fval)] = fval[(~np.isnan(fval))]
				const_flow[basin]=fval
				# from flux .th
				inds=(flux_indices[np.asarray(basinbdrivers[basin])][0])
				ivalid=~np.isnan(inds)
				inds=np.asarray(inds[ivalid],int)
				from_bdriver[ivalid]=-np.mean(q[Ryears==years[0]][:,inds],axis=0)#[0]
				from_bdriver
				flux_inds[basin]=inds
			else: 	
				from_bdriver=[]
	...
	return river_inflow,Ryears,bd_rnames,src,src_indices,bc_indices,flux_inds,const_flow	
...
if sel_sub:
	year_means=ds['schism'].sel(time=slice('20160901','20170901'))
	for varname in schism_vars[1:]:
		...
		if len(year_means[varname].shape)==3:
			schism_means[varname]=np.asarray(year_means[varname][:,:,-1].mean(axis=0)*factor)
		else:		
			schism_means[varname]=np.asarray(year_means[varname][:].mean(axis=0)*factor)
	...
	river_inflow,Ryears,bd_rnames,src,src_indices,bc_indices,flux_inds,const_flow=get_annual_mean_river_runoff(s,areas,years,startdate=reftime)	
	# from flu.th
	schism_means['total_flux']=year_means['solar_radiation']+year_means['total_heat_flux']  # including solar 
	schism_means['total_flux']=schism_means['total_flux'].mean(axis=0)  # including solar 
	Q=np.loadtxt('/sciclone/home10/yinglong/DISKS/vims20/BlackSea/RUN25b/flux.th')
	Qtime=Q[:,0]
	Q=Q[:,1:]
	Q=np.abs(Q)
	Qdates=reftime  + dt.timedelta(seconds=np.diff(Qtime)[0])*np.arange(len(Qtime))
	Qdates=np.asarray(Qdates,np.datetime64)
	keep= (ds['schism']['time'][0].values<=Qdates) & (ds['schism']['time'][-1].values>Qdates) 
	Qdates=Qdates[keep]
	Q=Q[keep,:]
	Qmean=Q.mean(axis=0)
	river_inflow2=dict.fromkeys(river_inflow.keys())
	for key in river_inflow.keys():
		inds=bc_indices[key]-1
		Qmean[flux_inds[key]-1]
		ivalid=~np.isnan(const_flow[key])
		river_inflow2[key]=np.hstack((Qmean[flux_inds[key]-1],-const_flow[key][ivalid]))
	river_inflow=river_inflow2				
else: # take years as are
...
if not use_atmo_from_schism:
	...
	if sel_sub:
		...
		for basin in areas:
			trimeansP=schism_means['precipitation'][faces[eleminds[basin]]].mean(axis=1) # jahresmittel
			trimeansE=schism_means['evaporation'][faces[eleminds[basin]]].mean(axis=1)
			water_from_atmo[basin]=[(trimeansP*A[eleminds[basin]]).sum()] # m^3/a
			water_to_atmo[basin]=[(trimeansE*A[eleminds[basin]]).sum()] # m^3/a

			# heatflux by basin		
			for varname in heat_by_basin.keys():
				if varname=='total_flux':
					trimeans=schism_means[varname].values[faces[eleminds[basin],:]].mean(axis=1)
				else:
					trimeans=schism_means[varname][faces[eleminds[basin],:]].mean(axis=1)
				heat_by_basin[varname][basin]=[(trimeans*A[eleminds[basin]]/A[eleminds[basin]].sum()).sum()] # W/mm/a
...
if sel_sub:	# reduce to substted interval
	date0=ds['schism']['time'][0].values
	date1=ds['schism']['time'][-1].values
	fluxdates64=np.asarray(fluxdates,np.datetime64)
	keep=(date0<=fluxdates64) & (date1>=fluxdates64)
	for key1 in flux1.keys():
		for key2 in flux1[key1].keys():
			print(key1 + ' ' + key2)
			if (ntypes==1) & (key2!= 'tot'):
				flux1[key1][key2]=flux1['vol']['tot']*np.nan
			else:
				flux1[key1][key2]=flux1[key1][key2][keep,:]
...
if sel_sub:
	...
	sheet1 = wb.add_sheet('Budget {:s}_{:s}'.format(str(ds['schism']['time'][0].values)[:10],str(ds['schism']['time'][-1].values)[:10]))  # add_sheet is used to create 
	...
	for icol,basin in enumerate(areas):
		sheet1.write(irow, col0+icol, river_inflow[basin].sum()*scale2year*m3tokm3)
		#mat[irow,icol]=river_inflow[basin][np.where(years==year),:].sum()*scale2year*m3tokm3
		
	irow=labell.index('precipitation')	
	for icol,basin in enumerate(areas):
		sheet1.write(irow, col0+icol,water_from_atmo[basin][0]*scale2year*m3tokm3) #
	irow=labell.index('evaporation')	
	for icol,basin in enumerate(areas):
		sheet1.write(irow, col0+icol,-np.abs(water_to_atmo[basin][0])*scale2year*m3tokm3) #*scale2year

		# add strait labels # this does not accpint direction prescriped
		for icol,strait in enumerate(locnames):
			#sheet1.write(0, len(areas)+1+icol, strait)
			if 	locdir[icol]==-1:
				sheet1.write(row_flux_neg, len(areas)+1+icol, -flux1['vol']['pos'][:,icol].mean()*scale2year*m3tokm3)	
				sheet1.write(row_flux_pos, len(areas)+1+icol, -flux1['vol']['neg'][:,icol].mean()*scale2year*m3tokm3)	
				sheet1.write(row_flux_tot, len(areas)+1+icol, -flux1['vol']['tot'][:,icol].mean()*scale2year*m3tokm3)	
			else:
				sheet1.write(row_flux_pos, len(areas)+1+icol, flux1['vol']['pos'][:,icol].mean()*scale2year*m3tokm3)	
				sheet1.write(row_flux_neg, len(areas)+1+icol, flux1['vol']['neg'][:,icol].mean()*scale2year*m3tokm3)	
				sheet1.write(row_flux_tot, len(areas)+1+icol, flux1['vol']['tot'][:,icol].mean()*scale2year*m3tokm3)	
		...
		for icol,basin in enumerate(areas):
			...
			if not BasinStraits[basin]==None:
				for item in BasinStraits[basin]:
					...
					if item[1]==-1:
						posflux+=-flux1['vol']['neg'][:,coli].mean()
						negflux+=-flux1['vol']['pos'][:,coli].mean()
						totflux+=-flux1['vol']['tot'][:,coli].mean()
					else:
						posflux+=flux1['vol']['pos'][:,coli].mean()
						negflux+=flux1['vol']['neg'][:,coli].mean()
						totflux+=flux1['vol']['tot'][:,coli].mean()
		...
		for icol,basin in enumerate(areas):
			Net[basin]=0
			Net[basin]+=(water_from_atmo[basin][0]-np.abs(water_to_atmo[basin][0]))#*scale2year*m3tokm3)
			Net[basin]+=river_inflow[basin].sum()#*scale2year*m3tokm3
			totflux=0
			if not BasinStraits[basin]==None:
				for item in BasinStraits[basin]:
					coli=locnames.index(item[0])
		...
		for varname in heat_by_basin.keys():
			...
			for icol,basin in enumerate(areas):
				sheet1.write(irow, col0+icol,heat_by_basin[varname][basin][0])
