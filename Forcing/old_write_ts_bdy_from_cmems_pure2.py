...
# prepreare SCHISM boundary forcing for a year based on cmems
...
mon0=6
zlvls=[]
setupdir='/gpfs/work/jacobb/data/SETUPS/EuropeJoseph/' #'/gpfs/work/jacobb/data/RUNS/Europe/' #'/gpfs/work/jacobb/data/SETUPS/BlackSea/'
sourcedir= '/gpfs/work/jacobb/data/DATA/Forcing/mount_cmems/nudge_start_jun2012/' #'/gpfs/work/jacobb/data/DATA/Forcing/mount_cmems2/'
#sourcedir='/mnt/lustre01/work/gg0028/g260114/Data/Forcing/cmems_dayly/nudge_start_jun2012/' #/gpfs/work/jacobb/data/DATA/Download/motuclient-python-v1.8.4/blacksea/'
#sourcedir=sourcedir0+str(year)+'/'
cwd=os.getcwd()

startdate=dt.datetime(2012,6,1)
#enddate=dt.datetime(2012,9,30)
enddate=dt.datetime(2013,12,31)
...
output_dt=86400 # time to interpolate to
...
different_files_for_vars=False
...
#nrdays=nrdays+(year%4==0)-offset#+1 # 366.0	# forcing interval for days
...
Dt=(nc[1].variables['time'][-1]-nc[0].variables['time'][0])
...
# data properties
#t0=-Dt/2 #43200.0  # start time relative to model start
#dt=86400.0	  # data time interval

#timesq=np.arange(offset*dt,(nrdays+(year%4==0))*dt+dt,dt)*output_dt
#timesq=np.arange(offset*output_dt,(nrdays+(year%4==0))*dt+dt,dt)*output_dt
#np.arange(startdate,enddate,dt.tim
timesq=np.arange(0,nrdays*86400+3600,output_dt)

ut = utime('seconds since {0}-{1:02d}-01 00:00:00'.format(year,mon0)) # SCHISM Time referenced
...
# output times
#timesq=np.arange(0,nrdays*86400+3600,3600)
