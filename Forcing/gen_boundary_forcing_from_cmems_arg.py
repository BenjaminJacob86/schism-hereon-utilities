...
openbd_segs=[0,1,2,3] 										  # boundary segments to create forcing fore (start counting from 0)
...
datestr1=sys.argv[1]
datestr2=sys.argv[2]
startdate=dt.datetime.strptime(datestr1,'%Y%m%d')
enddate=dt.datetime.strptime(datestr2,'%Y%m%d')
print('generate forcing for {:s}  to {:s}'.format(datestr1,datestr2))

#startdate=dt.datetime(2012,6,1)								  # simulation start date	
#enddate=dt.datetime(2012,7,31)								  # simulation endate date	
#enddate=dt.datetime(2014,12,31)								  # simulation endate date	
...
different_files_for_vars=False								  #  tem/salt etc in different files?
