...
t0=dt.datetime(2017,1,2)		#  time start WK
t1=dt.datetime(2017,1,8)		#  time end WK
year=str(t1.year)
#dt_output=86400    # timestep for output [seconds] take as input
dt_output=0 #3600         # if zero take same temporal resolution as input data
...
frocingtype='custom'#'gcoast' # IBI # CMEMS
frcdir='/work/gg0028/g260114/mengyao/forcing_by_file/'
...
name_time='time'
name_depth='depth'
fmt='%Y%m%d'     # start file date format
if frocingtype=='gcoast':
	...
	temp_files=''
	uv_pattern=''
elif frocingtype=='custom': 
	name_ssh='zos'
	name_salt='so'
	name_temp='thetao'
	name_u='uo'
	name_v='vo'
	#ssh_pattern='zos'   # pattern of file names
	#temp_pattern='thetao'
	#salt_pattern='so'
	#uv_pattern='uo'
	wild_elev_files='*zos*.nc' # define here unique wild cards contained in the filenamse of your forcing 
	wild_veloU_files='*uo*.nc'	
	wild_veloV_files='*vo*.nc'	
	wild_temp_files='*thetao*.nc'	
	wild_salt_files='*so*.nc'	
	name_lon='longitude'
	name_lat='latitude'
	name_time='time'    
	name_depth='depth'
	fmt = '%Y-%m-%d' # #fmt='%Y%m%d' #start date in fules
...
def get_file_start_dates(files,fmt='%Y%m%d'):
	start_dates=[]
	for i,file in enumerate(files):
		...
		start_dates.append(pd.to_datetime(file[i0:i1], format=fmt)) #%H%M
		#np.datetime64(file[i0:i1]))
	return(start_dates)
    
# modfiy this    
if frocingtype=='custom':
    def get_file_start_dates(files,fmt='%Y%m%d'):
        start_dates=[]
        for i,file in enumerate(files):
            if i==0:
...
if frocingtype=='IBI':
	...
	try:
		...
		def date_from_name(files):
			return np.asarray([ np.int(file[file.rindex(date_prefix)+len(date_prefix):file.rindex('.')].replace('-','')) for file in files])
...
if (frocingtype!='IBI') or (frocingtype!='custom'):		
...
if frocingtype=='IBI':
	...
	def reduce_file_list(files,fmt=fmt):
		""" return file list and start date for IBI files """
		npt0,npt1=np.asarray([t0,t1],np.datetime64)		
		file_dates=np.asarray(get_file_start_dates(files))
		#file0=int(np.maximum(np.where(file_dates>=npt0)[0][0]-1,0)) # has day before included
...
for count,type_file in enumerate(zip(ssh_files[:1],temp_files[:1],salt_files[:1],u_files[:1],v_files[:1])):
	...
	for key in grid.keys():
		...
		if len(grid[key][name_lon].shape) == 1:
			lon2d,lat2d=np.meshgrid(grid[key][name_lon],grid[key][name_lat])
			grid[key][name_lon]=lon2d.copy()
			grid[key][name_lat]=lat2d.copy()
...
#lon2d=grid['ssh'][name_lon]
#lat2d=grid['ssh'][name_lat]
#
#if len(lon2d.shape) == 1:
...
## u ######
lon2d=grid['u'][name_lon]
lat2d=grid['u'][name_lat]
...
## v ######
lon2d=grid['v'][name_lon]
lat2d=grid['v'][name_lat]
...
### same grid
fidx=0
ds={'ssh':xr.open_mfdataset(ssh_files[:2]),'salt':xr.open_mfdataset(salt_files[:2]),'temp':xr.open_mfdataset(temp_files[:2])}
if dt_output == 0:
...
nt_file=len(xr.open_dataset(ssh_files[0]).time) #24
...
depthin=ds_temp[name_depth].values
