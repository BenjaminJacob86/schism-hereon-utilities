...
%
% 12.12.2019 Benjamin Jacob
%  - implement correct reading of quads
%  - instead of hard coded FES directory add variables for FES elevation
%  and current pathes in user input pard
%  - To deal with river boundaries potentially in hgrid.gr3 add array specifing for which  
%  boundaries to apply tidal forcing and which are rivers
%
% Doesent work propberly if quads in grid and distinguish river boundaries  
%
...
%/gpfs/home/hosseini/PGOS_2D_R2/gpfs/home/jacobb/PGtides/
...
%% 0 B) where is the FES data

% FES elevetion directory @ level containing  direcetory "ocean_tide_extrapolated"
fes_elevetion_dir='/gpfs/work/jacobb/data/DATA/Tides/fes2014_elevations_and_load/fes2014b_elevations_extrapolated'; 
% FES current directory contains subfolders estward_velocity &  northward_velocity 
fes_current_dir='/gpfs/work/jacobb/data/DATA/Tides/fes2014a_currents/';
% leave fes_current_dir=''; if only using tidal forcing
% FES elevetion directory (contains direcetory ocean_tide_extrapolated)
...
ops_river=[0 2 1 2]; % river boundary format
 ...
 % Boundary NRs for which to apply tidal forcing
forcing_boundaries=[1]; 

apply_tidal_forcing=false(size(bounodes)); %
apply_tidal_forcing(forcing_boundaries)=1;
...
if apply_tidal_forcing(hh) % for forcing boundaries
...
else % for different boundaries
fprintf(fid,'%d %d %d %d %d %s %d %s   \n', length(bounodes{hh}), ops_river, '!', hh,'river boundary');

if ops_river(2)==2
    fprintf(fid,'%s\n','-1.2345 ! Const river discharge');
end 
...
function [amp,phase ]= gen_harm_FES_SCHISM(iflag, const,pts_data,fes_elevetion_dir,fes_current_dir)
% iflag=2;
...
for i=1:length(const)
  if(ifl==1)
    ncid = netcdf.open(fullfile(fes_elevetion_dir,fullfile('ocean_tide_extrapolated', [const{i} '.nc'])),'NC_NOWRITE');
  elseif(ifl==2)
    ncid = netcdf.open(fullfile(fes_current_dir,fullfile('eastward_velocity/', [const{i} '.nc'])),'NC_NOWRITE');
  elseif(ifl==3)
    ncid = netcdf.open(fullfile(fes_current_dir,fullfile('northward_velocity/', [const{i} '.nc'])),'NC_NOWRITE');
  else
    error('Unknown iflag')
  end
...
%     lat2=open(j,3);
    ...
    if(amp_max>100)
      if(amp_min>100 || amp_min<0)
        ...
        % error chaeck look one further
        disp('All junks for amp: search in second order region')
        %amp_min=min([amp0(I+[-1:+1],J+[-1+1]));
        
        amp_min=min(min(amp0(I+[-1:+1],J+[-1:+1]))); % 2 minimun in second order region
        
        amp(j,i,ifl)=amp_min; % use min
        if(amp_min>100 || amp_min<0)
...
%    end
    ...
    if(pha_max>370) %junk
      if(pha_min<0 ||  pha_min>370)
        ...
        pha_min=min(min(pha0(I+[-1:+1],J+[-1:+1]))); % 2 minimun in second order region
