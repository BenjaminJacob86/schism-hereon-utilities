...
if frocingtype=='IBI':
	...
	def reduce_file_list(files):
		...
		file0=int(np.maximum(np.where(file_dates>=npt0)[0][0]-1,0))
...
for k in range(LON.shape[0]):
	...
	#
	#plt.plot(LON[check],	LAT[check],'r+')
	...
	plt.figure()
	plt.pcolormesh(LON,LAT,dsi.ssh[0,:])
	
	data=dsi.ssh[0,:].values
	
	vint,bw1,bw2,bw3,bw4=bilin3(LON,LAT,data,indsx,indsy,bdx,bdy,True) #get bilinear weights


	plt.clf()
	plt.pcolormesh(LON,LAT,dsi.ssh[0,:],vmin=-1,vmax=1)
	plt.colorbar()
	#plt.scatter(bdlon,bdlat,s=24,c=vint,vmin=-1,vmax=1,edgecolors='k')
	plt.scatter(bdlon,bdlat,s=24,c=vint,vmin=-1,vmax=1,edgecolors=None)
	plt.plot(bdlon,bdlat,'r+')
	
	get_4neighbours(LON,LAT,bdlon,bdlat)
	...
	plt.pcolormesh(LON,LAT,a)
	plt.plot(LON[check].flatten(),LAT[check].flatten(),'r+')
	...
	for count,type_file in enumerate(zip(ssh_files,temp_files,salt_files,u_files,v_files)):
		...
		if count==0: #determine grids
			...
			start_dates=get_file_start_dates(ssh_files)
	import dask
	dask.config.set(**{'array.slicing.split_large_chunks': True})

	ds={'ssh':xr.concat([xr.open_dataset(file) for file in ssh_files],dim="time_counter",data_vars=['ssh']),
	'salt':xr.concat([xr.open_dataset(file) for file in salt_files],dim="time_counter",data_vars=['so']),
	'temp':xr.concat([xr.open_dataset(file) for file in temp_files],dim="time_counter",data_vars=['thetao']),
	}
	dsu=xr.concat([xr.open_dataset(file) for file in u_files], dim="time_counter")
	dsv=xr.concat([xr.open_dataset(file) for file in v_files], dim="time_counter")
	ds['u']=dsu
	ds['v']=dsv	


	 start_dates[0] > np.datetime64('20230101')
		...
		#'salt':xr.concat([xr.open_dataset(file).chunk() for file in salt_files], dim='time'),
		#}
		dsu=xr.concat([xr.open_dataset(file).chunk() for file in u_files], dim='time')
		dsv=xr.concat([xr.open_dataset(file).chunk() for file in v_files], dim='time')
		#ds['uv']=xr.concat([dsu,dsv], dim='time')
		ds['u']=dsu
		ds['v']=dsv
...
if frocingtype=='IBI':
	...
	grid=dict.fromkeys(['ssh','salt','temp','u','u'])
else:
...
if frocingtype == 'IBI':
	...
	# stupid ibi netcdf formatting needs manual indicce computations	

	ds['ssh']=ds['ssh'].sel(nav_lon=slice(lon.min()-1,lon.max()+1),nav_lat=slice(lat.min()-1,lat.max()+1))
