...
for mon in range(mon0,mon1+1):
	...
	for i,expi in enumerate(results.keys()):
		ndry+=access[i].get('dryFlagNode').sel(time=slice(t[index][0],t[index][-1])).values
	ndry=ndry>0
	
	filenameout=prefix+str(years[0])+'_{:02d}.nc'.format(mon) # outputfile
	for ivar,vari in enumerate(list(results[expi].keys())):   # variavle loop
		
		#if vari =='depthAverageVel':
		#	break
		...
		# 2D  or surface + bottom  fields
			...
			for irun in range(nrun):
				...
				if vari in vector_vars: # vector variables -calc absolute value
					isvec=True
					if nrun==1:						
						data=access[i].get(vari).sel(time=slice(t[index][0],t[index][-1])).values
					else:
					...
					if nrun==1:						
						...
						data=results[expi][vari][quanti]=access[i].get(vari).sel(nSCHISM_vgrid_layers=levels[irun],time=slice(t[index][0],t[index][-1])).values
