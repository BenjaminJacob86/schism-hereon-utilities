...
imax=np.argmax(hottime_wwm) # latest entry is not necessary the maximum
#date=np.asarray(hottime_wwm[-1],dt.datetime) # not the later one
#date=np.asarray(hottime_wwm[imax],dt.datetime) # not the later one
#strdate=str(date)
...
# check match if not latest
if hottime_wwm[imax] == hottime_schism[0]:
	imatch=imax
elif hottime_wwm[0] == hottime_schism[0]:
	imatch=0
elif hottime_wwm[-1] == hottime_schism[0]:
	imatch=-1

date=np.asarray(hottime_wwm[imatch],dt.datetime) # not the later one
strdate=str(date)
strdate=strdate.replace('-','')[:8]	
	
	
if hottime_wwm[imatch] == hottime_schism[0]: # matching hotstarts
	...
	with open('wwminput.nml','w') as fout:	
		fout.writelines(lines)		

else:

		
	with open('RUN_STOPPED','w') as fout:	
		fout.write('hotstarttime mismatch between schism ({:s}) and wwm ({:s})'.format(hottime_wwm[imax],hottime_schism[0]))
