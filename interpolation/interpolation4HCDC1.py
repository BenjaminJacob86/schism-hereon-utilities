...
add_depth=False # add depths variable
...
varfiles = dict.fromkeys(['out2d','horizontalVelX','horizontalVelY'])
for key in varfiles:
...
varnames = ['elevation', 'horizontalVelX', 'horizontalVelY',
            'sigWaveHeight','bottomStressX','bottomStressY']
...
for idate in range(len(varfiles['out2d'])):
    ...
    if (not os.path.exists(outfile)) or overwrite:
        ...
        t0 = time[0]
        try:
            seconds = (time - t0) / np.timedelta64(1, 's') # date format
        except:  # not date formatW
            seconds = (time - 0)
            t0=reftime
