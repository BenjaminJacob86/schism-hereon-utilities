...
def load_wwm_site_multiple_files(files,varlist=False,write_joint_file=False):
		...
		with open(file) as f:
			...
			for line in f.readlines():
				...
				for index in ireplace:
					...
					#check two decimal points in line for inrpoper split
				ls=line.split()
				for icol,item in enumerate(ls):
					if item.count('.')>1:
						print('2 .  in number -> wrong formmat')
						#from IPython import embed; embed()	
						
						a=item[:item.index('.')+9] # 8 digits aftert .
						b=item[item.index('.')+9:]
						ls= ls[:icol]+[a,b]+ls[icol+1:]
				for d,val in zip(list(data.keys()),ls):
	...
	for d in (data.keys()):
		try:
			...
			print(d)
			#from IPython import embed; embed()	       
			data[d]=np.ones(len(unqindex))*-9999 #np.asarray(data[d],float)[unqindex]
			print('Error reading WWM line format in column' + d )
	min=(data['TIME'])*100
