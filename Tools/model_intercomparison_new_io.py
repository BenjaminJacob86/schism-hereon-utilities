...
# SCHIMS grid files
#setupdir=['/work/gg0028/g260114/SETUPS/NWBS/setup/shelfOnlyCompact/']
# bdshift vs other convigs
#setupdir=['/work/gg0028/g260114/SETUPS/NWBS/setup/newcode/NWBS_danube_repair/NWBS_vgrid_re/']
##setupdir+=['/work/gg0028/g260114/SETUPS/NWBS/setup/newcode/NWBS_danube_repair/NWBS_vgrid_re/shap_max/']
...
#setupdir+=['/work/gg0028/g260114/SETUPS/NWBS/setup/newcode/NWBS_bdshift/']
...
#names=['CMEMS','NWBS_vgr_re','GaussFilter-wk_shp','NWBS_BDshift',]
#
# bdshift compate
setupdir=['/work/gg0028/g260114/SETUPS/NWBS/setup/newcode/NWBS_bdshift/']
setupdir+=['/work/gg0028/g260114/SETUPS/NWBS/setup/newcode/NWBS_bdshift/niter_shap2/']

setupdir+=['/work/gg0028/g260114/SETUPS/NWBS/setup/newcode/NWBS_bdshift/nudged/']
#setupdir+=['/work/gg0028/g260114/SETUPS/NWBS/setup/newcode/NWBS_bdshift/nudged_iunder/']
ncdir=[setupdir[0]+'all_out/']#'outputs/'] # where the outputs are.
#ncdir+=[setupdir[1]+'all_oout/']#+'outputs/'] # where the outputs are.
ncdir+=[setupdir[1]+'outputs/']#+'outputs/'] # where the outputs are.
ncdir+=[setupdir[2]+'outputs/']#+'outputs/'] # where the outputs are.
#ncdir+=[setupdir[2]+'outputs/'] # where the outputs are.
#names=['CMEMS','bdshift','nudged','nudged_iunder',]
names=['CMEMS','bdshift','shapiter2','nudged']
...
#ncdir+=[setupdir[1]+'outputs/'] # where the outputs are.
#ncdir+=[setupdir[2]+'outputs/'] # where the outputs are.
#ncdir+=[setupdir[2]+'outputs_shaprioNorm/'] # where the outputs are.
#ncdir+=[setupdir[3]+'outputs/'] # where the outputs are.
#ncdir+=[setupdir[4]+'outputs/'] # where the outputs are.
#ncdir=[setupdir[0]+'outputs_hotstart_not_freshened/']

#outdir='/work/gg0028/g260192/SEARECAP/validationTest'

outdir=setupdir[0]+'imageComp_bd_shift/' #'/work/gg0028/g260114/postproc/modelcomp/comp4/'
if not os.path.exists(outdir): os.mkdir(outdir) 

#names=['CMEMS','NWBS','NWBS_vgr_re','NWBS_vgr_re_shapiro','GaussFilter-wk','GaussFilter-wk_shp','GaussFilter-strng',]
...
class cmems():
	...
	def get_slab(self, time, varname, ndims=4, layer=0):
		...
		self.slab=np.asarray(self.slab)		
...
uvfiles=np.sort(glob.glob('/work/gg0028/g260114/SETUPS/NWBS/setup/Forcing/data/nrt.cmems-du.eu/Core/BLKSEA_ANALYSISFORECAST_PHY_007_001/all/*CMCC--RFVL-BS*.nc'))
     
uv=xr.open_mfdataset(uvfiles)	 

uvi=uv.sel(time='2021-11-29')

U=uvi['uo'][0,0,:].values
V=uvi['vo'][0,0,:].values
	

LON,LAT=np.meshgrid(models[0].lon,models[0].lat)
UVabs=np.sqrt(U**2+V**2)
plt.pcolormesh(LON,LAT,UVabs)
n=3
...
n_models_half=int(np.ceil(nx/2))
n_models_m1=int(nx-1)


update_colorrange=True


 models[2].nc['out2d']['time'][-1].values
...
#	# plot values
	...
	count=len(models)-1
	th1=[]
	for i,model in enumerate(models[:n_models_m1]): #int(np.ceil(nx/2))
		for j, model2 in enumerate(models):
			#if model2 != model:
			if j > i:
				th1.append([])
	...
	for i,name in enumerate(names):
		...
		exec('tsphs.append(axes[1][0].plot(np.asarray(tdata[name]),np.asarray(ydata[name]),".-",linewidth=2))'.format(i))
...
#					inr+=1
#					if not 'schism' in str(type(model)):
#						#if not 'schism' in str(type(model2)): #case comparing structured grid models not pallanned now
#						#	pass
#						#else:
#							
#							#inr+=1
...
#						#inr+=1
#						updata[inr]=(np.ma.masked_array(model2.slab[model2.nn[i]],mask=model2.masks[i])-model.slab)
#		print('update difference data '+str((dt.datetime.now()-t0).total_seconds())+' s')
		...
		model=models[0]
		i=0
		# difference to first structured grid model
		for j, model2 in enumerate(models[1:]):
			inr+=1
			updata[inr]=(np.ma.masked_array(model2.slab[model2.nn[i]],mask=model2.masks[i])-model.slab)
			
			
			#if structi:
			#	tdata[name].append(model.t)
			#	ydata[name].append(model.slab.flatten()[nns[name]])
			#else:		
			#	tdata[name].append(model.ti)
			#	ydata[name].append(model.slab[nns[name]])			
			
			
		for i,model in enumerate(models[2:n_models_m1]): #int(np.ceil(nx/2))
			for j, model2 in enumerate(models[i+1:]):
				inr+=1
				updata[inr]=(np.ma.masked_array(model2.slab[model2.nn[i+1]],mask=model2.masks[i+1])-model.slab).flatten() #[0,:]
		print('update difference data '+str((dt.datetime.now()-t0).total_seconds())+' s')		
		...
		#
		#t0=dt.datetime.now()
		##write biase mae leaf out for speed now
		for phi,data,structi,model in zip(phs,updata,struct,pmodels):
			#l=+1
			if structi==1:
				phi.set_array(data.flatten()) #.values
			else:
				#phi.set_array(data[s.nvplt[:,:3]].mean(axis=1)) check check check
								#phi.set_array(data[:-1,:-1].values.flatten())
				# mask drs
				#data.mask
				phi.set_array(np.ma.masked_array(data[model.nvplt[:,:3]].mean(axis=1),mask=model.wetdry+(time> model.dates[-1]))) #ignore mask
		...
		#print('update tool '+str((dt.datetime.now()-t0).total_seconds())+' s')
		...
		#reference to cmems maybe faster
		
		if update_colorrange:
			check=updata[0][updata[0].mask==False]
			#vmin,vmax=np.floor(np.nanquantile(updata[0],0.1)),np.ceil(np.nanquantile(updata[0],0.99))
			vmin,vmax=np.floor(np.nanquantile(check,0.1)),np.ceil(np.nanquantile(check,0.99))
			#[0]
			
			
			# bypass this, necessary?
			for i,model in enumerate(models):
				#plt.subplot(ny,ny,i+1)
				if not 'schism' in str(type(model)):
					#ttls[i]=plt.title(names[i]+ ' ' + str(model.t)[5:])
...
#		g3.set_ydata(deltay)
		...
		#for name,tsphi in (zip(names,tsphs)):
		#	tsphi[0].set_xdata(np.asarray(tdata[name]))
		#	tsphi[0].set_ydata(np.asarray(ydata[name]))
		#print('update ts '+str((dt.datetime.now()-t0).total_seconds())+' s')	
		#
		
		t0=dt.datetime.now()
		for name,model,tsphi,structi in zip(names,models,tsphs,struct):
			#if not 'schism' in str(type(model)):
			if structi:
				tdata[name].append(model.t)
				ydata[name].append(model.slab.flatten()[nns[name]])
		...
		# check how to optimiue bltting
		fig.canvas.draw()
		#patchAxes[i].draw_artist(patchAxes[i].tripcolor)
		#patchAxes
