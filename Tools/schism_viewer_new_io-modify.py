#!/usr/bin/env python
""" 
GUI Tool for SCHISM netcdf out (schout_nc) visualization.!vi 
...
class Window(tk.Frame):
    plt.ion() # notwendig ?    
    def __init__(self,master=None,ncdirsel=None):
    ...
    def schism_plotAtelems(self,nodevalues):
        ph=plt.tripcolor(self.x,self.y,self.faces[:,:3],facecolors=self.nodevalues[self.faces[:,:3]].mean(axis=1),shading='flat',cmap=plt.cm.jet)# shading needs gouraud to allow correct update
        ch=plt.colorbar()
        plt.tight_layout()
        return ph,ch
    ...
    def schism_updateAtelems(self):
        ...
        if self.shape==(self.nt,self.nnodes,self.nz):
            if self.CheckFixZ.get()==0:
                ...
                self.nodevalues=weights[0,:]*self.ncs[self.vardict[self.varname]][self.varname][self.total_time_index,:,:][self.nodeinds,ibelow]+weights[1,:]*self.ncs[self.vardict[self.varname]][self.varname][self.total_time_index,:,:][self.nodeinds,iabove]
                self.nodevalues=np.ma.masked_array(self.nodevalues,mask=np.isnan(self.nodevalues))
            title=self.varname
            #u=self.ncv['hvel'][self.total_time_index,:,self.lvl,0]
            #v=self.ncv['hvel'][self.total_time_index,:,self.lvl,1]
            ...
            title=self.varname
            #u=self.ncv['hvel'][self.total_time_index,:,0]
            #v=self.ncv['hvel'][self.total_time_index,:,1]
        elif self.shape==(self.nt,self.nnodes,2):
            #u=self.ncs[self.vardict[self.varname]][self.varname][self.total_time_index,:,0]
            #v=self.ncs[self.vardict[self.varname]][self.varname][self.total_time_index,:,1]
            #self.nodevalues=np.sqrt(u*u+v*v)
            title='abs' + self.varname
        elif self.shape==(2,self.nt,self.nnodes,self.nz):

            if self.CheckFixZ.get()==0:
                u=self.ncs[self.vardict[self.varname]][self.varname][0,self.ti,:,self.lvl].values
                v=self.ncs[self.vardict[self.varname]][self.varname][1,self.ti,:,self.lvl].values
            else:
                u=weights[0,:]*self.ncs[self.vardict[self.varname]][self.varname][0,self.ti,:,:][self.nodeinds,ibelow]+weights[1,:]*self.ncs[self.vardict[self.varname]][self.varname][0,self.ti,:,:][self.nodeinds,iabove]
                v=weights[0,:]*self.ncs[self.vardict[self.varname]][self.varname][1,self.ti,:,:][self.nodeinds,ibelow]+weights[1,:]*self.ncs[self.vardict[self.varname]][self.varname][1,self.ti,:,:][self.nodeinds,iabove]
            self.nodevalues=np.sqrt(u*u+v*v)
            self.nodevalues=np.ma.masked_array(self.nodevalues,mask=np.isnan(self.nodevalues))
        ...
        if self.varname != 'depth':
            title+=' @ ' + str(self.reftime + dt.timedelta(seconds=np.int(self.ncs['out2d']['time'][self.total_time_index]))) + ' level= ' + str(lvl)
        
        # setting colorbar
        #from IPython import embed; embed()
        self.ph.set_array(np.ma.masked_array(self.nodevalues[self.faces[:,:3]].mean(axis=1),mask=self.dryelems==1)) 
        if not self.CheckVar.get():
            self.clim=(np.nanmin(self.nodevalues),np.nanmax(self.nodevalues))
            self.minfield.delete(0,tk.END)
            self.maxfield.delete(0,tk.END)
            self.minfield.insert(8,str(np.nanmin(self.nodevalues)))
            self.maxfield.insert(8,str(np.nanmax(self.nodevalues)))
        else:
    ...
    def init_window(self,ncdirsel=None):
        ...
        # load files    
        if ncdirsel==None:
            print("naviagate into schout_*.nc directory")
        ...
        print('found ' +str(self.nstacks) +' files')

		
        # file access        
		# vars
        varfiles=[file[file.rindex('/')+1:].split('_')[0] for file in glob.glob(self.combinedDir+'*_'+str(nrs[0])+'.nc') ]
     ...
     'sigma_theta_f', 'sigma_maxdepth', 'Cs', 'dryFlagElement'] # exclude for plot selection
        ...
        w3=tk.OptionMenu(self, self.variable, *self.varlist) #w3.config(width=10)
        w3.grid(row=row,column=0)

		
        #levels=range(self.ncv['zcor'].shape[2])[::-1] #dropdown
        levels=[int(k) for k in range(self.ncs[self.vardict['zCoordinates']]['zCoordinates'].shape[2])] #spinwheel
        #self.lvl_tk = tk.IntVar(self)
        self.lvl_tk = tk.IntVar(self)
        #self.lvl_tk.set(levels[0]) # default value # dropdown
        self.lvl_tk.trace("w",self.lvl_callback)
        self.lvl_tk.set(str(np.max(levels))) # spinwheel default value
        #self.lvl=levels[0]  # dropdown
        self.lvl=levels[-1]   # spinwheel
        #w3=tk.OptionMenu(self, self.lvl_tk, *levels)
        #w4=tk.Spinbox(self, values=levels, width=5,validate='all',textvariable=self.lvl_tk) # try command and set		
    ...
    def variable_callback(self,*args):    # select variable
        ...
        if self.varname=='depth': # all vector here
            ...
            self.ivs=1			
        elif (self.varname =='hvel') or (self.varname =='windSpeed'): # all vector here			
            self.ivs=2		
        else:
            self.ivs=1
        #from IPython import embed; embed()	# this concatted hvel stored in
    ...
    def ti_callback(self,*args): #set timestep
        ...
        self.total_time_index=self.ti+self.stack_size*(self.current_stack-1) # expressed via stack and timstep within stack
    ...
    def get_layer_weights(self,dep): 
        ...
        a=np.sum(zcor<=dep,1)
        #print(zcor.mask.shape)  # masked levels below bottom layer
    ...
    def stream2image(self,*args):
        ...
        for nr,fig in enumerate(plt.get_fignums()):
            plt.figure(fig)
            plt.savefig(self.streamdir+'_{:d}_'.format(nr)+'{0:05d}'.format((self.stack.get()-1)*self.nt+self.ti)+'_'+self.varname+'.png',dpi=300)
    ...
    def timeseries(self,coords=None):
        ...
        if self.shape==(self.nt,self.nnodes,self.nz): #3D
            ...
            if self.npt==1:
                self.ts=ncs[self.vardict[self.varname]][self.varname][i0:i1,self.nn,:].values.reshape(i1-i0,1,2)
            else:
                self.ts=ncs[self.vardict[self.varname]][self.varname][i0:i1,self.nn,:].values
        elif self.shape==(2,self.nt,self.nnodes,self.nz):
            if self.npt==1:
    ...
    def hovmoeller(self,coords=None):
        ...
        self.zcor=np.squeeze(self.ncs[self.vardict['zCoordinates']]['zCoordinates'][i0:i1,self.nn,:])    #np.squeeze(self.nclv['zcor'][i0:i1,self.nn,:])
        self.t=self.nclv['time'][i0:i1]/86400
        ...
        if self.ivs==1:
            ...
            for iplt in range(1,3):
                plt.subplot(3,1,iplt)
                plt.pcolor(np.tile(self.t,(self.nz,1)).transpose(),self.zcor,np.squeeze(self.ts[:,:,iplt-1]))#/86400
                plt.ylabel('depth')
                plt.title(self.varname)
                plt.colorbar()
            ...
            plt.subplot(3,1,3)
            plt.pcolor(np.tile(self.t,(self.nz,1)).transpose(),self.zcor,np.sum(np.sqrt(self.ts**2),axis=-1))#/86400
    ...
    def plot_transect(self,dataTrans):		
        plt.pcolor(self.xs,self.zi,dataTrans) #shading=['flat','faceted'][self.meshVar.get()]
    ...
    def transect_callback(self,coords=None):
        ...
        if coords==None:
            ...
            self.zi=np.zeros((len(self.parents),self.nz))#*np.nan
        w=self.ndeweights
        zcor=self.ncs['zCoordinates']['zCoordinates'][self.total_time_index,:].values #nan and zeros
        ...
        #plt.pcolor(self.xs,self.zi,dataTrans)
        ...
        if self.ivs==1: # scalar # interpolation along sigma layers
            ...
            for i in range(len(self.parents)):
                ...
                self.zi=np.ma.masked_array(self.zi,mask=self.dataTrans.mask)
            self.plot_transect(self.dataTrans)
        else: # vector
            ...
            vert=self.ncs['vertical_velocity']['vertical_velocity'][self.total_time_index,:]
            print(vert.reshape((1,self.nnodes,self.nz)).shape)
            print(data.shape)
            data=np.concatenate((data,vert.reshape((self.nnodes,self.nz,1))),axis=2) # stack vertil velocity
            for i in range(len(self.parents)):
                depths=zcor[self.faces[self.parents[i],:],:]
                lvldata=data[self.faces[self.parents[i],:],:]
            ...
            for iplt in range(0,2):
                ...
                ch=self.plot_transect(self.dataTrans[:,:,iplt])
            ...
            plt.subplot(3,1,3)
            ch=self.plot_transect(np.sqrt(self.dataTrans[:,:,0]**2+self.dataTrans[:,:,1]**2))
    ...
    def savecoords(self,*args):
            ...
            np.savetxt(outname,mat,fmt='%i %.6f %.6f %i')
