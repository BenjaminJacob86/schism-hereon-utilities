...
class Window(tk.Frame):
    ...
    def find_parent_tri(self,tris,xun,yun,xq,yq,dThresh=1000):
        ...
        self.ibttms=np.asarray([(self.nc['node_bottom_index'][self.faces[parent[i],:]]).max()-1 for i in range(len(parent)) ],int)
    ...
    def schism_updateAtelems(self):
        ...
        if self.shape==(self.nt,self.nnodes,self.nz):
            if self.CheckFixZ.get()==0:
                self.nodevalues=self.ncv[self.varname][self.ti,:,self.lvl]
            else: # z interpolation
                self.nodevalues=weights[0,:]*self.ncv[self.varname][self.ti,:,:][self.nodeinds,ibelow]+weights[1,:]*self.ncv[self.varname][self.ti,:,:][self.nodeinds,iabove]
                self.nodevalues=np.ma.masked_array(self.nodevalues,mask=np.isnan(self.nodevalues))
            title=self.varname
            u=self.ncv['hvel'][self.ti,:,self.lvl,0]
            v=self.ncv['hvel'][self.ti,:,self.lvl,1]

        elif self.shape==(self.nnodes,):
            self.nodevalues=self.ncv[self.varname][:]
            title=self.varname
            self.quiver=0

        elif self.shape==(self.nt,self.nnodes):
            self.nodevalues=self.ncv[self.varname][self.ti,:]
            title=self.varname
            u=self.ncv['hvel'][self.ti,:,0]
            v=self.ncv['hvel'][self.ti,:,1]
        elif self.shape==(self.nt,self.nnodes,2):
            u=self.ncv[self.varname][self.ti,:,0]
            v=self.ncv[self.varname][self.ti,:,1]
            self.nodevalues=np.sqrt(u*u+v*v)
            title='abs' + self.varname
        elif self.shape==(self.nt,self.nnodes,self.nz,2):
            if self.CheckFixZ.get()==0:
                u=self.ncv[self.varname][self.ti,:,self.lvl,0]
                v=self.ncv[self.varname][self.ti,:,self.lvl,1]
            else:
    ...
    def init_window(self,ncdirsel=None):
        ...
        self.faces=self.ncv['SCHISM_hgrid_face_nodes'][:]-1
        self.x=self.ncv['SCHISM_hgrid_node_x'][:]
        self.y=self.ncv['SCHISM_hgrid_node_y'][:]
        self.nt,self.nnodes,self.nz,=self.ncv['zcor'].shape
        ...
        ##########################################  
        self.dryelems=self.ncv['wetdry_elem'][0,:][self.origins]
     ...
     'SCHISM_hgrid_node_y', 'node_bottom_index', 'SCHISM_hgrid_face_x', 'SCHISM_hgrid_face_y', 
     ...
     'sigma_theta_f', 'sigma_maxdepth', 'Cs', 'wetdry_elem'] # exclude for plot selection
        varlist=[]
        for vari in self.ncv.keys():
        ...
        w3=tk.OptionMenu(self, self.variable, *varlist) #w3.config(width=10)
        w3.grid(row=row,column=0)

		
        #levels=range(self.ncv['zcor'].shape[2])[::-1] #dropdown
        levels=[int(k) for k in range(self.ncv['zcor'].shape[2])] #spinwheel
        ...
        self.ti=0
        w2=tk.OptionMenu(self, self.ti_tk, *range(len(self.ncv['time'])))
    ...
    def variable_callback(self,*args):    # select variable
        ...
        if self.varname!='depth':
            self.ivs=self.ncv[self.varname].ivs
        else:
            self.ivs=1
        self.shape=self.ncv[self.varname].shape        
    ...
    def get_layer_weights(self,dep): 
        ...
        ibelow=a+(self.nc['node_bottom_index'][:]-1)-1
    ...
    def transect_callback(self,coords=None):
        ...
        if coords==None:
            ...
            self.zi=np.zeros((len(self.parents),self.nz))
			
        w=self.ndeweights
        zcor=self.nc['zcor'][self.ti,:]
        data=self.nc[self.varname][self.ti,:]
        for i in range(self.nz):
            self.zi[:,i]=(zcor[:,i][self.faces[self.parents]]*w).sum(axis=1)
        ...
        if self.ivs==1: # scalar
            ...
            self.dataTrans=np.zeros((len(self.parents),self.nz,self.ivs+1))
            vert=self.nc['vertical_velocity'][self.ti,:]
            print(vert.reshape((self.nnodes,self.nz,1)).shape)
            print(data.shape)
            data=np.concatenate((data,vert.reshape((self.nnodes,self.nz,1))),axis=2) # stack vertil velocity
            for i in range(len(self.parents)):
                depths=zcor[self.faces[self.parents[i],:],:]
                lvldata=data[self.faces[self.parents[i],:],:]
                ibttm=(self.nc['node_bottom_index'][self.faces[self.parents[i],:]]).max()-1 # add to coordinate part
