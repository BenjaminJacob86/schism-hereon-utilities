...
setupdir=[basedir+exp+'/'  for exp in experiments]
ncdir=[basedir+exp+'/outputs_merged/'  for exp in experiments]
names=experiments

use_amm=False # if False only compate schism model setups amongst each other
...
outdir=setupdir[0]+'imageComp/' 
if not os.path.exists(outdir): os.mkdir(outdir) 


varnames=['Hs','ssh']	#varname ['ssh',] if only one has to have ,
#varnames=['temp','salt']
sdictvnames = {'temp':'temperature','ssh':'zCoordinates','salt':'salinity','Hs':'sigWaveHeight'}
min_max={'ssh':(-1,1),'salt':(0,25),'temp':(5,25)}	# axis range for variables
difflims={'ssh':1,'salt':4,'temp':4,'Hs':1}     # # axis limits +- in difference plot
dthours={'ssh':3,'salt':12,'temp':12,'Hs':6}	# make plot each dthours hour
ndays_ts={'ssh':2,'salt':30,'temp':30,'Hs':2}             # nr of days depicted in time series subplot

# considrede time periods and steps for vriables
year0=2017
year1=2017
vartimes={'ssh':{'startdate':dt.datetime(year0,1,2,1,0),'enddate':dt.datetime(year1,12,30,1,0),'step[hours]':1},\
'Hs':{'startdate':dt.datetime(year0,1,2,1,0),'enddate':dt.datetime(year1,12,30,1,0),'step[hours]':1},\
'salt':{'startdate':dt.datetime(year0,1,2,12,0),'enddate':dt.datetime(year1,2,1,12,0),'step[hours]':24},\
...
for i_setup,diri in enumerate(setupdir):
	...
	for key in files.keys():
		files[key]=np.hstack([np.sort(glob.glob('{:}/{:s}_{:s}.nc'.format(ncdir[i_setup],key,'?'*iorder))) for iorder in range(1,6)])
