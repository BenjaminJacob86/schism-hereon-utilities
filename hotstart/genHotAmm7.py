...
rundir=os.getcwd()
destination_dir=rundir#rundir[:rundir.rindex('/')]+'/' #assume one above this directory
#destination_dir='/gpfs/work/jacobb/data/RUNS/GermanBight/GermanBight_2017_2018/'
datestr='20111101' # overwrite programitcally or make input parameter
hotoutname='GB_hot_AMM7'+datestr+'.nc' 
sigma=True   # True: Sigma  Flase: LSC2
nsigma=21
plothot=True

frcdir=destination_dir+'forcing/download_cmems_GB/'
frcdir='/gpfs/work/pein/storage/MyOcean_3layer_ab2011/'

# files with different priorities
#/gpfs/work/pein/storage/MyOcean_3layer_ab2011/metoffice_foam1_amm7_NWS_PSAL_b20131101_hi20131031.nc
#/gpfs/work/pein/storage/MyOcean_3layer_ab2011/metoffice_foam1_amm7_NWS_TEMP_b20131101_hi20131031.nc

# amm files as source
ids=['amm7_3']
type={ids[0]:'amm7_3'}
fileS=frcdir+'metoffice_foam1_amm7_NWS_PSAL_b20131101_hi20131031.nc'
fileT=frcdir+'metoffice_foam1_amm7_NWS_TEMP_b20131101_hi20131031.nc'
#metoffice_foam1_amm15_NWS_TEM_b20170103_hi20170102.nc
#fileT='/gpfs/work/ksddata/model/ocean/AMM15/metoffice_foam1_amm15_NWS_TEM_'+datestr
#fileS='/gpfs/work/ksddata/model/ocean/AMM15/metoffice_foam1_amm15_NWS_SAL_'+datestr
files={ids[0]:{'fileS':fileS,'fileT':fileT}}
prio={ids[0]:1}

#prio={'schism':1}  #  prio,value := priority of dataset to use in interpolaton. 0:= dont use this type. n=1,2,...N: if node of schism setup to interpolate to falls into domain
...
sigma_in=True   # True: Sigma  Flase: LSC2
#nsigma_in=21
...
if len(srcdata) > 1:
	...
	contains.append(np.ones(lon.shape,bool))
	
take_from=-np.ones(stp.nnodes,int)
for i,content in enumerate(contains):
	take_from[content & (take_from==-1) ]=i
plt.clf()


# read map where to use which hotstart
...
if plothot:
	ph,ch,ax=stp.plotAtnodes(take_from)
