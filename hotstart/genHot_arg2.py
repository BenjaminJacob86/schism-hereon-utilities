...
#srcdir='/gpfs/work/jacobb/data/RUNS/routine_GB_wave/Downloads/Download/download_cmems_GB/'
#rundir='/gpfs/work/jacobb/data/RUNS/routine_GB_wave/'
#date='20210804'

#################### Settings ########################################
# output setup - create a hot start for this setup
hotoutname='GB_hot_{:s}.nc'.format(date) 
...
ids=['GBAmm15']
modeltype={ids[0]:'amm'}
fileS='{:s}metoffice_foam1_amm15_NWS_SAL_{:s}.nc'.format(srcdir,date)  
fileT='{:s}metoffice_foam1_amm15_NWS_TEM_{:s}.nc'.format(srcdir,date) 
files={ids[0]:{'fileS':fileS,'fileT':fileT}}
prio={ids[0]:1}

# use existent schism hotstart to interpolate for  the hotstart bein created here 
#ids+=['schism']
#fileS=source_dir='/gpfs/work/jacobb/data/SETUPS/GermanBight/GB2017/'
#fileT='/gpfs/work/jacobb/data/SETUPS/GermanBight/GB2017/hotstart_it=31104.nc'
#files[ids[1]]={'fileS':fileS,'fileT':fileT}
#modeltype[ids[1]]='schism'
#prio[ids[1]]=2

#prio={'schism':1}  #  prio,value := priority of dataset to use in interpolaton. 0:= dont use this modeltype. n=1,2,...N: if node of schism setup to interpolate to falls into domain
...
# objects
sources={id:{'modeltype':modeltype[id],'files':files[id],'prio':prio[id]} for id in ids}
...
for src in sources.keys():
	print(src)
	if sources[src]['modeltype'] == 'schism':
		#os.chdir(source_dir)
		...
		uselist+=['schism']
	elif sources[src]['modeltype'] == 'amm':
		fileS=sources[src]['files']['fileS']
		fileT=sources[src]['files']['fileT']
...
if 0:				
	...
	if prio['schism']!=0:
		...
		for isrc,src in enumerate(srcdata):
			...
			if not 'schism' in str(src): #str(type[src])!="<class '__main__.schismhot'>":			
