...
!     Open nc file 
!     ncfile1='tsfiles/TS_'//char3(1:len_char3)//'.nc'  !TS_1.nc .... TS_n.nc
      ncfile1='/gpfs/work/jacobb/data/SETUPS/NorthSea/NorthSea200mHelgoland/nudging/tsfiles/S_'//char3(1:len_char3)//'.nc'  !TS_1.nc .... TS_n.nc
      ncfile2='/gpfs/work/jacobb/data/SETUPS/NorthSea/NorthSea200mHelgoland/nudging/tsfiles/T_'//char3(1:len_char3)//'.nc'  !TS_1.nc .... TS_n.nc
      print*, 'doing ',trim(ncfile1)
      status = nf90_open(trim(ncfile1),nf90_nowrite, sid)
      status = nf90_open(trim(ncfile2),nf90_nowrite, tid)
      call check(status)

!     Get dimnesions from first file, assumed same for all variables not the
!     case for amm15 2018
!      if(ifile==1) then
        status = nf90_inq_varid(sid, "so", svid)
        status = nf90_inq_varid(tid, "thetao", tvid)!temp is ncfile2
...
!       WARNING: indices reversed from ncdump!
        ...
        if(ifile==1) then ! shifted from above
...
!        status = nf90_get_var(sid, yvid, yind)
        ...
        status = nf90_get_var(sid, yvid, yind)
        write(20,*)'depth size',size(hnc)
        write(20,*)'lon size',size(xind)
        write(20,*),'lat size',size(yind)
...
!        cmems is reversed fisrt index is surface last is bottom
!        status = nf90_get_var(sid,svid,salt(:,:,ilen:1:-1),start=(/1,1,1,it2/),count=(/ixlen,iylen,ilen,1/))
!        status = nf90_get_var(sid,tvid,temp(:,:,ilen:1:-1),start=(/1,1,1,it2/),count=(/ixlen,iylen,ilen,1/))
        ...
        !Scaling etc
        salt=salt*1.e-3+30 !(0.00152592547238)-0.00152592547238  !*1.e-3+20
        temp=temp*1.e-3+25 !0.000732444226742+21!*1.e-3+20
        !Define junk value for sid; the test is
        !salt<rjunk+0.1
        rjunk=-32767*1e-3+30 !-32767*(0.00152592547238)-0.00152592547238   !-3.e4*1.e-3+20
